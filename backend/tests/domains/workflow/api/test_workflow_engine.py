...
def test_workflow_happy_path_manager_then_role() -> None:
    ...
    try:
        ...
        mgr_notifs = client.get(
            "/api/v1/notifications?unread_only=1&limit=50", headers=_auth(mgr_token)
        )
        assert mgr_notifs.status_code == 200, mgr_notifs.text
        assert any(
            n["type"] == "workflow.step.assigned" and n["entity_id"] == request_id
            for n in mgr_notifs.json()["data"]["items"]
        )
        ...
        hr_token = login(
            client, email=hr_admin["email"], password=hr_admin["password"]
        )["access_token"]
        inbox2 = client.get(
            "/api/v1/workflow/inbox?status=pending&limit=50", headers=_auth(hr_token)
        )
        ...
        hr_notifs = client.get(
            "/api/v1/notifications?unread_only=1&limit=50", headers=_auth(hr_token)
        )
        assert hr_notifs.status_code == 200, hr_notifs.text
        assert any(
            n["type"] == "workflow.step.assigned" and n["entity_id"] == request_id
            for n in hr_notifs.json()["data"]["items"]
        )
        ...
        assert any(
            n["type"] == "workflow.request.finalized" and n["entity_id"] == request_id
...
def test_workflow_tenant_isolation() -> None:
    ...
    try:
        ...
        link_user_to_employee(
            client, token=token_b, employee_id=emp_id, user_id=employee_b["user_id"]
        )
        ...
        get_other = client.get(
            f"/api/v1/workflow/requests/{request_id}", headers=_auth(token_a)
        )
        ...
        cleanup(
            engine,
            tenant_id=tenant_b["tenant_id"],
            user_ids=[admin_b["user_id"], employee_b["user_id"]],
        )
...
def test_workflow_concurrent_approve_race() -> None:
    ...
    try:
        ...
        hr1_token = login(client1, email=hr1["email"], password=hr1["password"])[
            "access_token"
        ]
        hr2_token = login(client2, email=hr2["email"], password=hr2["password"])[
            "access_token"
        ]
        ...
        with engine.begin() as conn:
            conn.execute(
                ...
                ),
                {
                    "id": file_id,
                    "tenant_id": tenant["tenant_id"],
                    "object_key": f"test/{file_id.hex}/test.txt",
                },
            )
