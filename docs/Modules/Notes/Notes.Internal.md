# Notes
    ...
    ## 1. Overview
    - Purpose: Fragmented notes surfaces, including ATS application notes and standalone notes routes that need verification.
    ...
    ## 2. Scope
    - In scope: Application notes, Notes library shell, Template shell.
...
- TODO: module-specific rule inventory should be extended where current code or docs are fragmented.
    ...
    - Primary workflow: use the module to complete application notes and related follow-up actions.
...
- Failure path: surface stable error code, correlation ID, and recovery guidance.
    ...
    ## 7. Screen Inventory
    - /notes - active route or known entry point.
- /notes/library - active route or known entry point.
- /notes/templates - active route or known entry point.
...
- Respect RTL, keyboard access, and current shell/navigation conventions.
    ...
    - Frontend entry points: /notes, /notes/library, /notes/templates.
- Referenced frontend sources: frontend/src/app/notes/*.
- State and data fetching should remain aligned with feature-first API wrappers and query-key patterns.
    ...
    - Referenced backend sources: backend/app/api/v1/ats.py.
